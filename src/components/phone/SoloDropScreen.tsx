import { ChevronLeft, Flame, Play, Coins, Zap } from "lucide-react";
...
const SoloDropScreen = () => (
  <div className="h-full overflow-y-auto scrollbar-hide pb-24 bg-background">
    ...
    <div className="relative mx-3 rounded-2xl overflow-hidden">
      <img src={album} alt="Keep Pushin" className="w-full aspect-square object-cover" loading="lazy" />
      <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-background via-background/80 to-transparent">
        <p className="font-display text-2xl text-glow">KEEP PUSHIN</p>
        <p className="text-xs text-muted-foreground">Young Legend</p>
      </div>
    ...
    <div className="mx-3 mt-3 rounded-xl bg-card border border-border p-3">
      <div className="flex items-center justify-between mb-2">
        ...
        <p className="text-xs font-bold text-accent">87%</p>
      </div>
      <div className="h-2.5 rounded-full bg-secondary overflow-hidden">
        <div className="h-full rounded-full transition-all" style={{ width: "87%", background: "linear-gradient(90deg, hsl(45 100% 60%), hsl(15 100% 55%))", boxShadow: "0 0 12px hsl(45 100% 60% / 0.7)" }} />
      </div>
    </div>
    ...
    <div className="grid grid-cols-3 gap-2 mx-3 mt-3">
      {[
        { label: "PLAYS", v: "12.6K" },
        { label: "LIKES", v: "2.3K" },
        { label: "TIPS", v: "850 CSB", gold: true },
      ].map((s) => (
    ...
    {/* Play */}
    <button className="mx-3 mt-3 w-[calc(100%-1.5rem)] py-3.5 rounded-xl bg-primary text-primary-foreground font-display text-lg flex items-center justify-center gap-2 glow-primary">
      <Play className="h-5 w-5 fill-current" /> PLAY SONG
    </button>

    {/* Tip + boost */}
    <div className="grid grid-cols-2 gap-3 mx-3 mt-3">
      <button className="py-3 rounded-xl bg-accent/15 border border-accent/40 text-accent font-bold text-sm flex items-center justify-center gap-2 animate-shake">
    ...
    <button className="mx-3 mt-3 w-[calc(100%-1.5rem)] py-3 rounded-xl bg-secondary text-left px-4 flex items-center justify-between text-sm">
      <span>Comments (128)</span>
